  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
        <h1 style={{ color: '#0D2B4E', margin: 0 }}>Job Listings</h1>
      ...
      {!jobs ? (
        ...
        <div className="card" style={{ marginTop: 20 }}>No roles posted yet.</div>
      ) : (
            ...
            <tbody>
              {jobs.map((j) => (
                <tr key={j.id} style={{ cursor: 'default' }}>
                  <td data-label="Role"><a href={`/admin/jobs/${j.id}/edit`} style={{ fontWeight: 700 }}>{j.title}</a></td>
                  ...
                  <td data-label="Actions" style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <a href={`/admin/jobs/${j.id}/edit`} className="btn btn-outline" style={{ padding: '6px 12px', minHeight: 32, fontSize: 12 }}>Edit</a>
                    {j.status === 'OPEN' ? (
                      <button
