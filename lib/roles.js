...
// Sales & CRM — Business Development owns the pipeline; Director has full
// access (matches the brief's role table's general Director-sees-everything
// rule, same pattern as RECRUITMENT_*/PROCUREMENT_* above).
const SALES_VIEW = ['DIRECTOR', 'BUSINESS_DEVELOPMENT'];
const SALES_MANAGE = ['DIRECTOR', 'BUSINESS_DEVELOPMENT'];

// Client Portal — CLIENT is the actual external login, strictly isolated to
// its own Client row server-side (never by client-supplied id — see
// app/api/portal/**). DIRECTOR/BUSINESS_DEVELOPMENT/BOARD_ADVISOR get the
// same screen as an internal multi-client view (a client picker), matching
// the existing isPortal screen's nav wiring for those three personas.
const PORTAL_VIEW = ['DIRECTOR', 'BUSINESS_DEVELOPMENT', 'BOARD_ADVISOR', 'CLIENT'];

// Finance — Accountant prepares/reconciles, Director has full access. Same
// shape as PAYROLL_VIEW/PAYROLL_PREPARE.
const FINANCE_VIEW = ['DIRECTOR', 'ACCOUNTANT'];
const FINANCE_MANAGE = ['DIRECTOR', 'ACCOUNTANT'];

// Contracts register — Director/Accountant manage it; HR Officer and
// Business Development need read access (they originate staff and client
// agreements respectively, per the brief, even though they don't edit the
// register directly).
const CONTRACTS_VIEW = ['DIRECTOR', 'ACCOUNTANT', 'HR_OFFICER', 'BUSINESS_DEVELOPMENT'];
const CONTRACTS_MANAGE = ['DIRECTOR', 'ACCOUNTANT'];

// Procurement receiving — posting to the expense ledger is a receiving-dock
// action, done by whoever requested/handles the physical delivery (IT &
// Facilities) or the Director; matches PROCUREMENT_REQUEST's role set.
const PROCUREMENT_RECEIVE = ['DIRECTOR', 'IT_FACILITIES'];

// Client Portal support requests — visible to the same roles as the rest of
// the Client Portal (PORTAL_VIEW); creating/replying is CLIENT (their own
// thread, always) plus the internal roles who actually work the Client
// Portal day to day (Director/Business Development) — Board Advisor stays
// strictly read-only, same as everywhere else it appears in this app.
const SUPPORT_VIEW = ['DIRECTOR', 'BUSINESS_DEVELOPMENT', 'BOARD_ADVISOR', 'CLIENT'];
const SUPPORT_REPLY = ['DIRECTOR', 'BUSINESS_DEVELOPMENT', 'CLIENT'];

// Invoices (brief section 3A) — billing register generated from real
// Client/Contract data. View is a bit broader than manage, same shape as
// CONTRACTS_VIEW/CONTRACTS_MANAGE just above: HR Officer is included in VIEW
// per the brief's role table listing Accountant/Director/HR for this area,
// even though only Accountant/Director actually progress an invoice's real
// status (draft -> sent -> paid) — HR can see billing exists without being
// able to change money-moving state, matching the Contracts precedent of
// "originates or needs visibility into it" vs. "actually manages it".
const INVOICES_VIEW = ['DIRECTOR', 'ACCOUNTANT', 'HR_OFFICER'];
const INVOICES_MANAGE = ['DIRECTOR', 'ACCOUNTANT'];

// Credentials Vault — restricted to Director and IT & Facilities ONLY, per
// the brief ("restricted to Director and IT & Facilities only"). No
// Accountant/HR/other-role access, unlike most other Phase 4 groups above.
const VAULT_VIEW = ['DIRECTOR', 'IT_FACILITIES'];
const VAULT_MANAGE = ['DIRECTOR', 'IT_FACILITIES'];
const VAULT_REVEAL = ['DIRECTOR', 'IT_FACILITIES'];
