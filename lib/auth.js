...
function createSessionToken(user) {
  const payload = {
    ...
    // Phase 4 (Client Portal) — set only for Role=CLIENT accounts. Every
    // portal-scoped route re-derives the caller's own clientId from THIS
    // verified cookie (never from a request body/query param), so a client
    // login can never see another client's data even by crafting a request
    // with a different client's id.
